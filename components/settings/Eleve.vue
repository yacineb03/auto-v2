  ...
  <div class="flex h-screen overflow-hidden bg-[#F8F8FA] text-neutral-900 font-sans">
    ...
    <aside class="hidden md:flex w-64 flex-col bg-white border-r border-neutral-100 h-full">
      ...
      <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
        <p class="px-4 text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-2">Menu</p>
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden relative">
      ...
      <div class="flex-1 overflow-y-auto w-full p-4 md:p-8 pb-24 md:pb-8">
        ...
        <div class="max-w-3xl mx-auto">
          <h1 class="text-2xl font-bold text-neutral-900 mb-6">Paramètres du compte</h1>

          <!-- Profile Card -->
          <div class="bg-white rounded-xl p-6 md:p-8 shadow-sm border border-neutral-100 mb-8">
            <div class="flex items-center gap-6 mb-8">
              <div class="relative group cursor-pointer">
                <div class="w-20 h-20 rounded-full bg-neutral-100 overflow-hidden border-4 border-white shadow-lg">
                  <div class="w-full h-full flex items-center justify-center bg-[#4F46E5] text-white text-2xl font-bold">
                    {{ user.initials }}
                  </div>
                </div>
                <div class="absolute inset-0 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <iconify-icon icon="solar:camera-linear" class="text-white" width="24"></iconify-icon>
                </div>
              </div>
              <div>
                <h2 class="text-xl font-bold text-neutral-900">{{ user.civility }} {{ user.firstname }} {{ user.lastname }}</h2>
                <p class="text-sm text-neutral-500">Élève • {{ user.email }}</p>
              </div>
            </div>
            ...
            <form class="space-y-6" @submit.prevent>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                  ...
                  <input type="text" :value="user.firstname" readonly class="w-full px-4 py-3 rounded-xl border border-neutral-200 bg-neutral-50 text-neutral-500 cursor-not-allowed">
                </div>
                <div class="space-y-2">
                  <label class="text-sm font-medium text-neutral-700">Nom</label>
                  <input type="text" :value="user.lastname" readonly class="w-full px-4 py-3 rounded-xl border border-neutral-200 bg-neutral-50 text-neutral-500 cursor-not-allowed">
                </div>
                <div class="space-y-2">
          ...
          <div class="bg-white rounded-xl p-6 md:p-8 shadow-sm border border-neutral-100 mb-8">
            <h3 class="text-lg font-bold text-neutral-900 mb-6 flex items-center gap-2">
              <iconify-icon icon="solar:shield-keyhole-linear" class="text-[#4F46E5]"></iconify-icon>
              Sécurité & Mot de passe
            </h3>

            <div class="space-y-4">
              <div class="space-y-2">
                <label class="text-sm font-medium text-neutral-700">Ancien mot de passe</label>
                <input type="password" class="w-full px-4 py-3 rounded-xl border border-neutral-200 focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/20 outline-none transition-all">
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-2">
                  <label class="text-sm font-medium text-neutral-700">Nouveau mot de passe</label>
                  <input type="password" class="w-full px-4 py-3 rounded-xl border border-neutral-200 focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/20 outline-none transition-all">
                </div>
                <div class="space-y-2">
                  <label class="text-sm font-medium text-neutral-700">Confirmer</label>
                  <input type="password" class="w-full px-4 py-3 rounded-xl border border-neutral-200 focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/20 outline-none transition-all">
                </div>
              </div>
               <div class="pt-4 flex justify-end">
                <button class="px-6 py-3 border border-neutral-200 text-neutral-600 font-semibold rounded-xl hover:bg-neutral-50 transition-colors">
          ...
          <!-- Notifications -->
          <div class="bg-white rounded-xl p-6 md:p-8 shadow-sm border border-neutral-100">
            <h3 class="text-lg font-bold text-neutral-900 mb-6 flex items-center gap-2">
              <iconify-icon icon="solar:bell-bing-linear" class="text-[#4F46E5]"></iconify-icon>
              Notifications
            </h3>
            ...
            <div class="space-y-4">
              <div class="flex items-center justify-between p-4 bg-neutral-50 rounded-xl">
                <div>
                  <p class="font-medium text-neutral-900">Rappels de leçon</p>
                  <p class="text-xs text-neutral-500">Recevoir un SMS 24h avant chaque leçon</p>
                </div>
               ...
               <div class="flex items-center justify-between p-4 bg-neutral-50 rounded-xl">
                <div>
                  <p class="font-medium text-neutral-900">Nouveaux documents</p>
                  <p class="text-xs text-neutral-500">Email quand un document est validé</p>
                </div>
                <label class="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" checked class="sr-only peer">
                  <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#4F46E5]"></div>
...
<script setup>
const { data: dashboard } = await useFetch('/api/dashboard/eleve')
const user = computed(() => dashboard.value?.user || { firstname: '', lastname: '', email: '', phone: '', civility: 'M.', initials: 'XX' })
